...
{
    public class OfficeToXPSOffice
    {
        ...
        private static OfficeToXpsConversionResult ConvertFromExcel(string sourceFilePath, ref string resultFilePath)
        {
            ...
            try
            {
                var pExportFormat = Exl.XlFixedFormatType.xlTypeXPS;
                var pExportQuality = Exl.XlFixedFormatQuality.xlQualityStandard;
                var pOpenAfterPublish = false;
                var pIncludeDocProps = true;
                var pIgnorePrintAreas = true;
                Exl.Application excelApplication = null;
                Exl.Workbook excelWorkbook = null;
                try
                {
                    excelApplication = new Exl.Application();
                }
                catch (Exception exc)
                {
                    return new OfficeToXpsConversionResult(ConversionResult.ErrorUnableToInitializeOfficeApp, "Exl", exc);
                }
                try
                {
                    try
                    ...
                    if (excelWorkbook != null)
                    {
                        ...
                        {
                            return new OfficeToXpsConversionResult(ConversionResult.ErrorUnableToExportToXps, "Exl", exc);
                        }
                    }
                    else
                    {
            ...
            {
                return new OfficeToXpsConversionResult(ConversionResult.ErrorUnableToAccessOfficeInterop, "Exl", exc);
            }
            resultFilePath = pExportFilePath;
            return new OfficeToXpsConversionResult(ConversionResult.OK, pExportFilePath);
        }
